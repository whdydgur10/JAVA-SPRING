drop trigger if exists updateLevel;
	...
	begin
		...
		if old.isDel = 'N' and new.isDel = 'Y' then
			if old.isCoupon = 'Y' then
				update project.coupon set purchaseNum = null where purchaseNum = new.num;
			end if;
            if old.isPoint = 'Y' then
				update project.user set point = point + new.usePoint where id = new.userId;
			end if;
        end if;
	end//
	delimiter ;
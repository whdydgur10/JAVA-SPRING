...
<div class="head" style="height:120px;">
	<div style="position:fixed;top:0;left:0;right:0;background-color:white;z-index:10;">
		<div class="logo" style="text-align: center;width:1400px;margin:0 auto;position: relative;">
			<a href="<%=request.getContextPath()%>/?mainCategory=<c:if test="${user.gender == 'M'}">M</c:if><c:if test="${user.gender == 'W'}">W</c:if><c:if test="${user == null}">MW</c:if>" style="color:rgb(33,51,87);font-size:80px;"><img src="<%=request.getContextPath()%>/resources/img/logo.jpg" style="height:106;position:absolute;left:500px;top:10px;"> 빠숑</a>
				<div class="use" style="position: absolute;right:0;top:0;">
				<c:if test="${user.auth == 1}"><a href="<%=request.getContextPath()%>/root/page">관리자페이지 | </a></c:if>
